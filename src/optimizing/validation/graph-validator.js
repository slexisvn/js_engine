...
import { computeDominators, dominates } from "../passes/dominators.js";
...
export function validateOptimizedGraph(graph, frameStates = []) {
  const errors = [];
...
function validateNodeOwnership(graph, errors) {
...
function validateControlFlow(graph, errors) {
...
function validateBlockEdges(block, errors) {
...
function validateBranchTerminator(block, terminator, blockIds, errors) {
  const expectedIds = new Set([terminator.props.trueBlock, terminator.props.falseBlock]);
...
function validateJumpTerminator(block, terminator, blockIds, errors) {
  const targetId = terminator.props.targetBlock;
...
function validateFrameStates(graph, frameStates, errors) {
...
function validateBlockParams(graph, errors) {
  for (const block of graph.blocks) {
    ...
    for (const param of block.params) {
      ...
      for (let i = 0; i < block.predecessors.length; i++) {
        ...
        if (
          ...
          edgeArgs[param.props.index] !== param.inputs[i]
        ) {
          errors.push(
            `B${block.id} v${param.id} input ${i} does not match B${pred.id}->B${block.id} edge arg ${param.props.index}`,
          );
        }
      }
    }
...
function validateUseLists(graph, errors) {
  const values = graphValues(graph);
  const valueSet = new Set(values);
  const expected = new Map(values.map((value) => [value, new Map()]));
...
function incrementUse(uses, use) {
  uses.set(use, (uses.get(use) || 0) + 1);
}

function countUses(uses) {
  const counts = new Map();
  for (const use of uses) incrementUse(counts, use);
  return counts;
}

function validateUseDefDominanceWith(graph, dominators, locations, errors) {
  for (const block of graph.blocks) {
    for (const node of block.nodes) {
...
function validateFrameStateValueDominanceWith(graph, dominators, locations, errors) {
  for (const block of graph.blocks) {
    for (const node of block.nodes) {
