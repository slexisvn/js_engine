...
export function eliminateRedundantChecks(graph) {
    ...
    for (const node of block.nodes) {
      ...
      if (key !== null) {
        if (seenChecks.has(key)) {
          ...
          replaceGraphFrameStateValue(graph, node, original);
          toRemove.push(node);
          elimCount++;
        } else {
...
export function rangeAnalysisAndBoundsCheckElimination(graph) {
  ...
  const getRange = (id) => ranges.get(id) || { min: NEG_INF, max: INF };
  ...
  for (const block of graph.blocks) {
    ...
    const trueBlockId = term.props.trueBlock;
    const falseBlockId = term.props.falseBlock;
  ...
  for (const block of graph.blocks) {
    ...
    if (alwaysTrue || alwaysFalse) {
      const targetBlockId = alwaysTrue
        ? term.props.trueBlock
        : term.props.falseBlock;
  ...
  const loopHeaderBlocks = new Set();
  for (const block of graph.blocks) {
  ...
  const arrayLengthNodes = new Map();
  for (const block of graph.blocks) {
    for (const node of block.nodes) {
    ...
    for (let i = 0; i < block.nodes.length; i++) {
      ...
      if (indexRange.min >= 0 && indexRange.max >= 0 && indexRange.max < INF) {
        ...
        if (bounded && indexRange.min >= 0) {
          ...
          replaceGraphFrameStateValue(graph, node, indexNode);
...
function removeDeadPureNodes(graph) {
  let changed = true;
  while (changed) {
    changed = false;
    const toRemove = [];
