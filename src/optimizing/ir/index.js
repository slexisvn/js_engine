...
function inferEffectKind(opcode, metadata = {}) {
...
export function irRequiresFrameState(node) {
...
export class CFGInstruction {
  constructor(opcode, metadata = {}) {
    ...
    this.effectKind = metadata.effectKind || inferEffectKind(opcode, metadata);
  ...
  addInput(value) {
    ...
    if (value && value.uses) value.uses.push(this);
  }

  replaceInput(index, value) {
    const old = this.inputs[index];
    if (old && old.uses) old.uses = old.uses.filter((u) => u !== this);
    this.inputs[index] = value;
    if (value && value.uses) value.uses.push(this);
  }

  toString() {
    const inputIds = this.inputs.map((n) => `v${n.id}`).join(", ");
    const propsStr = Object.entries(this.props)
      .filter(([k]) => k !== "effectKind")
      .map(([k, v]) => `${k}=${typeof v === "string" ? '"' + v + '"' : v}`)
      .join(", ");
    const fs = this.frameState ? ` {fs}` : "";
    const rep = this.props._rep ? ` :${this.props._rep}` : "";
...
export class CFGFunction {
  ...
  addDependency(kind, id, version = null) {
    ...
    for (const block of this.blocks) {
      for (const instr of block.nodes) {
        for (const input of instr.inputs) {
          if (input && input.uses) input.uses.push(instr);
        }
      }
    }
  }
...
export function irParameter(index) {
  return new IRNode(IR_PARAMETER, { index });
}

export function irConstant(value) {
  return new IRNode(IR_CONSTANT, { value });
}

export function irCheckMap(obj, expectedMapId, expectedMapVersion = null) {
  const node = new IRNode(IR_CHECK_MAP, { expectedMapId, expectedMapVersion });
  node.addInput(obj);
  return node;
}

export function irCheckSmi(value) {
  const node = new IRNode(IR_CHECK_SMI);
  node.addInput(value);
  return node;
}

export function irCheckNumber(value) {
  const node = new IRNode(IR_CHECK_NUMBER);
  node.addInput(value);
  return node;
}

export function irInt32Add(left, right) {
...
export function irInt32Sub(left, right) {
...
export function irInt32Mul(left, right) {
...
export function irInt32Div(left, right) {
...
export function irInt32Mod(left, right) {
...
export function irFloat64Add(left, right) {
...
export function irFloat64Sub(left, right) {
...
export function irFloat64Mul(left, right) {
...
export function irFloat64Div(left, right) {
...
export function irInt32Compare(op, left, right) {
...
export function irFloat64Compare(op, left, right) {
...
export function irLoadField(obj, offset) {
  const node = new IRNode(IR_LOAD_FIELD, { offset });
  node.addInput(obj);
  return node;
}

export function irPolymorphicLoad(obj, maps, offsets) {
  const node = new IRNode(IR_POLYMORPHIC_LOAD, { maps, offsets });
  node.addInput(obj);
  return node;
}

export function irPolymorphicStore(obj, maps, offsets, value) {
...
export function irStoreField(obj, offset, value) {
...
export function irGenericAdd(left, right) {
...
export function irGenericSub(left, right) {
...
export function irGenericMul(left, right) {
...
export function irGenericDiv(left, right) {
...
export function irGenericMod(left, right) {
...
export function irGenericCompare(op, left, right) {
...
export function irGenericGetProp(obj, propName) {
  const node = new IRNode(IR_GENERIC_GET_PROP, { propName });
  node.addInput(obj);
  return node;
}

export function irGenericSetProp(obj, propName, value) {
...
export function irGenericCall(callee, args) {
...
export function irCheckCallTarget(callee, expectedTarget) {
  const node = new IRNode(IR_CHECK_CALL_TARGET, { expectedTarget });
  node.addInput(callee);
  return node;
}

export function irCallKnownFunction(target, args) {
...
export function irGenericGetIndex(obj, index) {
...
export function irGenericSetIndex(obj, index, value) {
...
export function irInt32Or(left, right) {
...
export function irInt32Xor(left, right) {
...
export function irInt32Ushr(left, right) {
...
export function irInt32Not(value) {
  const node = new IRNode(IR_INT32_NOT);
  node.addInput(value);
  return node;
}

export function irFloat64Pow(left, right) {
...
export function irGenericBitand(left, right) {
...
export function irGenericBitor(left, right) {
...
export function irGenericBitxor(left, right) {
...
export function irGenericShl(left, right) {
...
export function irGenericShr(left, right) {
...
export function irGenericUshr(left, right) {
...
export function irGenericPow(left, right) {
...
export function irGenericBitnot(value) {
  const node = new IRNode(IR_GENERIC_BITNOT);
  node.addInput(value);
  return node;
}

export function irGenericInstanceof(left, right) {
...
export function irGenericIn(left, right) {
...
export function irReturn(value) {
  const node = new IRNode(IR_RETURN);
  node.addInput(value);
  return node;
}

export function irBranch(condition, trueBlock, falseBlock) {
...
export function irJump(targetBlock) {
  return new IRNode(IR_JUMP, { targetBlock: targetBlock.id });
}

export function irDeoptimize(reason) {
  return new IRNode(IR_DEOPTIMIZE, { reason });
}

export function irBox(value, fromType) {
  const node = new IRNode(IR_BOX, { fromType });
  node.addInput(value);
  return node;
}

export function irUnbox(value, toType) {
  const node = new IRNode(IR_UNBOX, { toType });
  node.addInput(value);
  return node;
}

export function irNot(value) {
  const node = new IRNode(IR_NOT);
  node.addInput(value);
  return node;
}

export function irNeg(value) {
...
export function irNewArray(elements) {
...
export function irNewRegex(constIdx) {
  return new IRNode(IR_NEW_REGEX, { constIdx });
}

export function irLoadLocal(slot) {
  return new IRNode(IR_LOAD_LOCAL, { slot });
}

export function irStoreLocal(slot, value) {
  const node = new IRNode(IR_STORE_LOCAL, { slot });
  node.addInput(value);
  return node;
}

export function irLoadGlobal(name) {
  return new IRNode(IR_LOAD_GLOBAL, { name });
}

export function irStoreGlobal(name, value) {
  const node = new IRNode(IR_STORE_GLOBAL, { name });
  node.addInput(value);
  return node;
}

export function irCheckArray(obj) {
  const node = new IRNode(IR_CHECK_ARRAY);
  node.addInput(obj);
  return node;
}

export function irCheckElementsKind(arrayObj, elementsKind) {
  const node = new IRNode(IR_CHECK_ELEMENTS_KIND, { elementsKind });
  node.addInput(arrayObj);
  return node;
}

export function irCheckBounds(index, arrayObj) {
...
export function irLoadArrayLength(arrayObj) {
...
export function irInt32Shl(left, right) {
...
export function irInt32Shr(left, right) {
...
export function irInt32And(left, right) {
