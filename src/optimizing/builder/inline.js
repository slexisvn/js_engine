...
function hasReadOnlyUpvalues(target) {
  if (!target.upvalues || target.upvalues.length === 0) return true;
  const instructions = target.instructions;
  if (!instructions) return false;
  for (const instr of instructions) {
    if (instr.opcode === bytecode.ROP_STA_UPVALUE) return false;
  }
  return true;
}
...
function canInlineTarget(target, compiledFn, argCount, graph) {
  if (!target) return false;
  if (target === compiledFn) return false;
  if (target.paramCount !== argCount) return false;
  if (
    target.upvalues &&
    target.upvalues.length > 0 &&
    !hasReadOnlyUpvalues(target)
  )
    return false;
...
export function selectInlineTarget(callHint, compiledFn, argCount, graph) {
  ...
  if (callHint.kind === FEEDBACK_HINT_MONOMORPHIC) {
    ...
    return { target, targets: null, reason: "inlined" };
  }
...
export function recordInlineDecision(callHint, kind, reason) {
  if (callHint && callHint.slot)
...
export function tryInline(
  ...
  const blockAccs = new Map();
  const blockRegsMap = new Map();
  ...
  const captureInlineFrameState = (bytecodeOffset, regs, stack) =>
    captureFrameStateWithCaller(
      targetFn,
      bytecodeOffset,
      regs,
  ...
  const compileInlineInstruction = (
    ...
    if (
      ...
      const fbIdx = instr.operands.length > 1 ? instr.operands[1] : -1;
      const left = inlineAcc;
      const right = inlineRegs.get(rhsReg) || ir.irConstant(undefined);
      const feedbackKind = numericFeedbackKind(feedback, fbIdx, "binary");
      ...
      if (feedbackKind === "smi") {
        ...
        if (instr.opcode === bytecode.ROP_ADD)
          ...
          result = ir.irInt32Div(checkLeft, checkRight);
    ...
    if (
      ...
      const right = inlineRegs.get(rhsReg) || ir.irConstant(undefined);
      const feedbackKind = numericFeedbackKind(feedback, fbIdx, "binary");
      const cmpOp = COMPARE_OP_MAP[instr.opcode];
    ...
    if (instr.opcode === bytecode.ROP_LDA_PROP) {
      ...
      const propertyHint = fbIdx >= 0 ? feedback.property(fbIdx) : null;
      const elementsHint = fbIdx >= 0 ? feedback.elements(fbIdx) : null;
      ...
      if (propName === "length" && elementsHint && elementsHint.lengthAccess) {
        ...
        if (protoDepths.every((depth) => depth === 0)) {
          const frameState = captureInlineFrameState(
            inlineBcIdx,
            inlineRegs,
            [inlineAcc],
    ...
    if (instr.opcode === bytecode.ROP_LDA_GLOBAL) {
      const name = targetFn.constants[instr.operands[0]];
  ...
  for (let i = 0; i < instructions.length; i++) {
    ...
    if (inlineBlockMap.has(i)) {
      ...
      if (!blockAccs.has(i)) {
        ...
        blockRegsMap.set(i, new Map(currentRegs));
      }

      currentBlock = targetBlock;

      if (blockAccs.has(i)) {
        inlineAcc = blockAccs.get(i);
        currentRegs = new Map(blockRegsMap.get(i));
      }
    }
    ...
    if (instr.opcode === bytecode.ROP_JUMP) {
      ...
      if (!blockAccs.has(target)) {
        ...
        blockRegsMap.set(target, new Map(currentRegs));
      }
    ...
    if (
      ...
      if (!blockAccs.has(i + 1)) {
        ...
        blockRegsMap.set(i + 1, new Map(currentRegs));
      }
