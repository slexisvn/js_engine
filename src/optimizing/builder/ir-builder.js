...
import { rememberIncomingState, restoreIncomingState } from "./cfg-state.js";
...
function compileInstruction(
  ...
  switch (op) {
    ...
    case bytecode.ROP_LDA_GLOBAL: {
      ...
      const name = compiledFn.constants[nameIdx];
    ...
    case bytecode.ROP_VOID: {
      block._lastAcc = ir.irConstant(undefined);
      break;
    }
    ...
    case bytecode.ROP_NEW: {
      ...
      if (inlineTarget) {
        ...
        if (ctorInfo && ctorInfo.length > 0) {
          ...
          const layout = ctorInfo.map((field) => {
