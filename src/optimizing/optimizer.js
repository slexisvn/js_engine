import { IRGraph } from "./ir/index.js";
...
export class SpeculativeOptimizer {
  ...
  compile(compiledFn) {
    ...
    tracer.jitCompile(compiledFn.name, "Starting speculative compilation");
    ...
    const findLoopsFn = (g) => findLoops(g);

    const rebuildAll = () => { graph.rebuildUses(); buildFrameStateIndex(graph); };
