...
describe("constantFolding", () => {
  describe("arithmetic", () => {
    ...
    it("folds Int32Div by zero to 0", () => {
      const { graph, block } = makeGraph();
      const a = irConstant(10);
  ...
  describe("unary folding", () => {
    it("folds Not of constant truthy to 0", () => {
      const { graph, block } = makeGraph();
    ...
    it("folds Not of constant 0 to 1", () => {
      const { graph, block } = makeGraph();
...
describe("strengthReduction", () => {
  ...
  it("reduces divide by power of 2 to shift right", () => {
    const { graph, block } = makeGraph();
    const p = graph.addParameter(0);
    ...
    strengthReduction(graph);
    const replaced = ret.inputs[0];
    expect(replaced.type).toBe("Int32Shr");
    expect(replaced.inputs[1].props.value).toBe(2);
  });

  it("reduces mod by power of 2 to bitwise and", () => {
    const { graph, block } = makeGraph();
    const p = graph.addParameter(0);
    ...
    strengthReduction(graph);
    const replaced = ret.inputs[0];
    expect(replaced.type).toBe(IR_INT32_AND);
    expect(replaced.inputs[1].props.value).toBe(15);
  });
