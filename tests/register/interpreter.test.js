...
describe("helpers", () => {
  ...
  describe("errorToTaggedValue", () => {
    it("unwraps RegisterMiniJITException", () => {
      const val = mkString("err");
      const result = errorToTaggedValue(new RegisterMiniJITException(val));
...
describe("RegisterInterpreter", () => {
  ...
  describe("runFrame - exception handling", () => {
    it("THROW without handler propagates", () => {
      ...
      fn.constants = [42];
      expect(() => interp.execute(fn)).toThrow(RegisterMiniJITException);
    });
